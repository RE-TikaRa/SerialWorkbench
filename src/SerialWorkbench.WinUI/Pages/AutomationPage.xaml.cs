...
public sealed partial class AutomationPage : Page
{
    ...
    {
        InitializeComponent();
        SequenceJson.Text = JsonSerializer.Serialize(new SerialSequenceDefinition("新建序列", [new SerialSequenceStep([], "hex", 0, 1, 0)]), new JsonSerializerOptions(JsonSerializerDefaults.Web) { WriteIndented = true });
    }
    ...
    public SerialSequenceDefinition Parse() => JsonSerializer.Deserialize<SerialSequenceDefinition>(DefinitionText, new JsonSerializerOptions(JsonSerializerDefaults.Web)) ?? throw new InvalidDataException("序列定义为空。");
